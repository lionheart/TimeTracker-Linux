...
class AppletGConfWrapper(object):
    def __init__(self, applet, schema_path, standalone_key):
        ...
        if self._base_path is not None:
            ...
            # NOTE: Don't need to apply schema here because the TimeTracker Applet schema file
            ...
            # we have to do this ourselves in order to receive GConf change notifications.
            self._client.add_dir(self._base_path, gconf.CLIENT_PRELOAD_RECURSIVE) 
    ...
    def add_notification(self, relative_key, callback, data=None):
        ...
        callback should look like: callback(GConfValue, data=None)
        """
    ...
    def _notification_callback(self, client, cnxn_id, entry, data=None):
        ...
        # gconf_value is of type GConfValue (gconf.Value)
        gconf_value = entry.get_value()
        
        # Ignore when gconf_value is None because that
