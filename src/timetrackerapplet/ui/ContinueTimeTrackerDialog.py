...
class ContinueTimeTrackerDialog(object):
    (STOP_TIMER, KEEP_PAUSED, CONTINUE_TIMER) = xrange(3)

    def __init__(self, glade_file_name, header_text, body_text):
        self._dialog = gtk.Dialog(_('Continue Tracking Time'),
                                  None,
                                  gtk.DIALOG_DESTROY_WITH_PARENT,
                                  (_('_Stop Tracking'), gtk.RESPONSE_NO,
                                   _('_Keep Paused'), gtk.RESPONSE_CLOSE,
                                   _('_Continue Tracking'), gtk.RESPONSE_YES))
    ...
    def get_response(self):
        ...
        if dialog_result == gtk.RESPONSE_YES:
            return ContinueTimeTrackerDialog.CONTINUE_TIMER
        elif dialog_result == gtk.RESPONSE_NO:
            return ContinueTimeTrackerDialog.STOP_TIMER
        else:
            return ContinueTimeTrackerDialog.KEEP_PAUSED
